...
import torch
import src.decoders.ensemble
import src.model_builder

import src.translate.beam
import src.inputters as inputters

from src.translate.beam_search import BeamSearch
from src.translate.random_sampling import RandomSampling
from src.utils.misc import tile, set_random_seed
from src.modules.copy_generator import collapse_copy_scores
...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    load_test_model = src.decoders.ensemble.load_test_model \
        if len(opt.models) > 1 else src.model_builder.load_test_model
    fields, model, model_opt = load_test_model(opt)

    scorer = src.translate.GNMTGlobalScorer.from_opt(opt)
    # scorer = None
...
class Translator(object):
    ...
    def translate(
        ...
        xlation_builder = src.translate.TranslationBuilder(
        ...
        for batch in data_iter:
            ...
            for trans in translations:
                ...
                response.append(' '.join(trans.pred_sents[0]))
