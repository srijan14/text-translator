...
class AudioEncoder(EncoderBase):
    ...
    def forward(self, src, lengths=None):
        ...
        batch_size, _, nfft, t = onmt.size()
        src = onmt.transpose(0, 1).transpose(0, 3).contiguous() \
        ...
        for l in range(self.enc_layers):
            ...
            memory_bank = memory_bank.transpose(0, 2)
            src = memory_bank
            t, _, num_feat = onmt.size()
            src = batchnorm(onmt.contiguous().view(-1, num_feat))
            src = onmt.view(t, -1, num_feat)
